...
#End Region
    ...
    Private Sub BT_Ok_Click(ByVal sender As Object, ByVal e As EventArgs) Handles ButtonOK.Click
        ...
        ' le port du proxy ne peut pas être à null sinon
        My.Settings.ProxyPort = ProxyPort.Text

        ' port du proxy
        My.Settings.tracelog = fichier_log_activé
        ...
        ' On va écrire l'intervalle de vérification dans le user.config
        With My.Settings
        ...
        ' on va écrire si l'option Ancienne interface est activée
        ' ou non et si la durée de 'Ce soir' ou 'Maintenant' à
        ' été modifiée alors on redémarre ZG
        With CheckBoxoldUI

            If .Checked AndAlso My.Settings.oldUI = 0 Then
                ' Si on a activé  l'état 3D alors qu il ne l était pas
            ...
            Else
                If .Checked = False AndAlso My.Settings.oldUI = 1 Then
                    ' si on a descativé l'etat 3D alors qu il etait activé
                    With My.Settings
                        .dureecesoir = tempsminimumcesoir
                        .dureemaintenant = tempsminimummaintenant
                        .oldUI = 0
    ...
    Public Sub New()
        ...
        If Not (Not datemajverif Is Nothing AndAlso String.IsNullOrEmpty(datemajverif)) Then
            ' On affiche la date de la dernière vérification dans les préférences
            datemajverif = My.Settings.datemajverif
        ...
        ' On va lire l'intervalle de vérification dans le user.config et on affiche dans les préférences
        With My.Settings
        ...
        ' On va lire si l'option ancienne interface est cochée
        With CheckBoxoldUI
        ...
        With CheckBoxtracelog
            Select Case My.Settings.tracelog
                Case True
                    .Checked = True
                    fichier_log_activé = True
                Case Else
                    .Checked = False
                    fichier_log_activé = False
            End Select
        End With
        ...
        If Not (Not datemajverif Is Nothing AndAlso String.IsNullOrEmpty(datemajverif)) Then
            ...
            ' On affiche la date de la prochaine vérification dans les préférences
        ...
        Try
            ...
            For Each FileInfo As FileInfo In DirInfo.GetFiles
                If FileInfo.Extension = ".wav" OrElse FileInfo.Extension = ".mid" OrElse FileInfo.Extension = ".mp3" Then
                    ' On affiche les fichiers wav, mid et mp3 trouvés dans le répertoire 'Media' 
    ...
    Private Sub CheckBoxResume_CheckedChanged(ByVal sender As Object, ByVal e As EventArgs) _
        ...
        ' Si Autoupdate est coché on active les autres groupbox et on met à jour l'image dans la StatusStrip
    ...
    Private Sub CheckBoxAutoverif_CheckedChanged(ByVal sender As Object, ByVal e As EventArgs) _
        ...
        ' Si Autoverif est coché on active les autres groupbox
    ...
    Private Sub ButtonRSSSound_Click(ByVal sender As Object, ByVal e As EventArgs) Handles ButtonRSSSound.Click
        Try
            ' le volume va de 0 (max) à -3000 (min)
    ...
    Private Sub TrackBarMasterVolume_MouseUp(ByVal sender As Object, ByVal e As EventArgs) Handles TrackBarMasterVolume.MouseUp
        ...
        Try
            ' On va écrire le volume de ZGuideTV.NET
            AudioMixerHelper.SetVolume(CInt(TrackBarMasterVolume.Value*6553.5))
        Catch ex As Exception
            'On évite que cela plante dans AudioMixerHelper !!!
        End Try
